"""
EPA Ultimate Strategy V3 - Regime & BTC Exit Protection
=========================================================
Variant of EPAUltimateV3 with additional protective exit mechanisms:

1. Regime Exit Protection: Exit trades when market turns unfavorable
   - Close longs when market becomes choppy/ranging
   - Protects gains from trend reversals

2. BTC Exit Tightening: Tighter exits when BTC shows weakness
   - Monitors BTC/USDT trend for altcoin trades
   - Reduces drawdown during BTC-led selloffs

Author: Emre Uludaşdemir
Version: 3.4.0 - Regime Exit + BTC Tight Exit Protection
"""
...
class EPAUltimateV3_RegimeBTC(IStrategy):
    """
    EPA Ultimate Strategy V3 - Regime Exit + BTC Protection Variant
    
    Same as EPAUltimateV3 but with additional exit protections:
    1. use_regime_exit: Exit when market regime turns unfavorable
    2. use_btc_exit_tightening: Tighter exits when BTC weakens
    
    These features aim to reduce stop_loss exits and improve drawdown.
    """
    ...
    # ==================== V2 PROTECTION FLAGS (ENABLED BY DEFAULT) ====================
    use_regime_exit = BooleanParameter(default=True, space='sell', optimize=False)
    use_btc_exit_tightening = BooleanParameter(default=True, space='sell', optimize=False)
    
    # Regime exit threshold - exit if choppiness exceeds this
    regime_exit_chop_threshold = DecimalParameter(55.0, 75.0, default=62.0, space='sell', optimize=True)
    
    # BTC weakness detection - tighten profit target when BTC below this EMA
    btc_weakness_ema_period = IntParameter(10, 30, default=21, space='sell', optimize=True)
    btc_tight_profit_target = DecimalParameter(0.02, 0.05, default=0.03, space='sell', optimize=True)
    ...
    def informative_pairs(self):
        """Higher timeframes for trend confirmation + BTC monitoring."""
        ...
        # Always monitor BTC for exit tightening
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """Calculate all indicators - EPA base + Kıvanç indicators + BTC monitoring."""
        ...
        # ==================== BTC MONITORING (for exit tightening) ====================
        if self.use_btc_exit_tightening.value and self.dp:
            btc_df = self.dp.get_pair_dataframe(pair='BTC/USDT', timeframe=self.timeframe)
            if len(btc_df) > 0:
                btc_df['btc_ema'] = ta.EMA(btc_df, timeperiod=self.btc_weakness_ema_period.value)
                btc_df['btc_weak'] = (btc_df['close'] < btc_df['btc_ema']).astype(int)
                btc_df['btc_strong'] = (btc_df['close'] > btc_df['btc_ema']).astype(int)
                
                dataframe = merge_informative_pair(
                    dataframe, btc_df[['date', 'btc_weak', 'btc_strong']],
                    self.timeframe, self.timeframe, ffill=True
                )
                # Remove the suffix for easier access
                dataframe['btc_is_weak'] = dataframe.get(f'btc_weak_{self.timeframe}', 0)
                dataframe['btc_is_strong'] = dataframe.get(f'btc_strong_{self.timeframe}', 1)
            else:
                dataframe['btc_is_weak'] = 0
                dataframe['btc_is_strong'] = 1
        else:
            dataframe['btc_is_weak'] = 0
            dataframe['btc_is_strong'] = 1
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        Same as EPAUltimateV3 - entries are identical.
        """
        ...
        # Store WAE confirmation for position sizing
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Exit signals based on trend reversal + regime protection.
        
        V2 Enhancements:
        - Regime Exit: Exit when market becomes choppy
        - Standard exit: Indicator reversals
        """
        
        # ==================== STANDARD EXITS (same as V3) ====================
        standard_exit_long = (
            (
        ...
        # ==================== REGIME EXIT (V2 ENHANCEMENT) ====================
        # Exit when market turns choppy (protects gains from trend degradation)
        regime_exit_long = pd.Series(False, index=dataframe.index)
        if self.use_regime_exit.value:
            regime_exit_long = (
                dataframe['choppiness'] > self.regime_exit_chop_threshold.value
            )
        
        # Combined exit signal
        dataframe.loc[
            (standard_exit_long) | (regime_exit_long),
            'exit_long'
        ] = 1
        ...
        if self.can_short:
            ...
            regime_exit_short = pd.Series(False, index=dataframe.index)
            if self.use_regime_exit.value:
                regime_exit_short = (
                    dataframe['choppiness'] > self.regime_exit_chop_threshold.value
                )
            ...
            dataframe.loc[
                (standard_exit_short) | (regime_exit_short),
                'exit_short'
            ] = 1
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if trade.is_short:
            ...
            if chandelier > 0:
                ...
                atr_stop = min(atr_stop, chandelier_stop)
        else:
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime,
        ...
        """
        Dynamic position sizing based on volatility and confirmations.
        Same as EPAUltimateV3.
        """
        dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
                    ...
                    **kwargs) -> Optional[str]:
        """
        Tiered exits + BTC weakness protection.
        
        V2 Enhancement: When BTC is weak, tighten profit targets for altcoins
        """
        dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
        ...
        # ==================== BTC EXIT TIGHTENING (V2 ENHANCEMENT) ====================
        # When BTC shows weakness, exit altcoin longs at lower profit
        if self.use_btc_exit_tightening.value:
            btc_is_weak = last_candle.get('btc_is_weak', 0)
            
            # If BTC is weak and we have profit, take it
            if btc_is_weak == 1 and pair != 'BTC/USDT':
                if current_profit >= self.btc_tight_profit_target.value:
                    return 'btc_weakness_exit'
        
        # ==================== STANDARD TIERED EXITS ====================
