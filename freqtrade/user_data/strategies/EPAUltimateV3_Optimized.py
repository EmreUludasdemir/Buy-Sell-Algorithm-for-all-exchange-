"""
EPA Ultimate Strategy V3 - OPTIMIZED V1 (Quick Wins)
====================================================
Based on EPAUltimateV3 with aggressive parameter optimization.

CHANGES FROM V3:
1. Tighter stoploss: -0.08 → -0.05 (faster loss cuts)
2. Aggressive ROI table: 20%/15%/10%/5% stepped
3. Trailing stop ENABLED for profit lock-in
4. Reduced protections for more trades

EXPECTED IMPROVEMENT: 7.75% → 12-15%

Author: Emre Uludaşdemir
Version: 3.4.0-Optimized - Quick Wins Applied
"""
...
class EPAUltimateV3_Optimized(IStrategy):
    """
    EPA Ultimate Strategy V3 - OPTIMIZED VERSION
    
    Quick Wins Applied:
    - Tighter stoploss (5% vs 8%) - Faster loss cuts
    - Aggressive ROI table - Higher profit targets initially
    - Trailing stop enabled - Lock in profits
    - Reduced cooldown - More trading opportunities
    """
    ...
    # ==================== OPTIMIZED ROI TABLE ====================
    # Aggressive initial targets, stepped down over time
    minimal_roi = {
        "0": 0.20,      # 20% target in first 4 hours (was 12%)
        "240": 0.15,    # 15% after 10 hours
        "480": 0.10,    # 10% after 20 hours
        "720": 0.05,    # 5% after 30 hours (exit)
    }

    # ==================== OPTIMIZED STOPLOSS ====================
    # Tighter stop for faster loss cuts: -8% → -5%
    stoploss = -0.05

    # Enable trailing stop for profit lock-in
    use_custom_stoploss = True
    
    # Trailing configuration - MORE AGGRESSIVE
    trailing_stop = True
    trailing_stop_positive = 0.02        # Trail at 2% profit
    ...
    # REDUCED Protections for more trades
    @property
    def protections(self):
        return [
            {
                "method": "CooldownPeriod",
                "stop_duration_candles": 6  # REDUCED: 12 → 6 (faster re-entry)
            },
            {
                "method": "StoplossGuard",
                "lookback_period_candles": 48,
                "trade_limit": 3,  # INCREASED: 2 → 3 (more tolerance)
                "stop_duration_candles": 12,  # REDUCED: 24 → 12
                "only_per_pair": False
            },
            {
                "method": "MaxDrawdown",
                ...
                "stop_duration_candles": 24,  # REDUCED: 48 → 24
                "max_allowed_drawdown": 0.15  # INCREASED: 0.12 → 0.15
            }
        ]
    ...
    # Confluence Settings - RELAXED for more trades
    min_kivanc_signals = IntParameter(2, 3, default=2, space='buy', optimize=True)  # REDUCED: 3 → 2
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Entry logic - RELAXED for more trades.
        
        Key changes:
        - min_kivanc_signals default: 2 (was 3)
        - Dynamic confluence still applies
        """
        ...
        min_signals_required = np.where(
            ...
            self.min_kivanc_signals.value  # Use parameter (default 2)
        )
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        TIGHTER than V3: Returns -5% or 2.5 ATR (whichever is wider).
        """
        ...
        # Calculate 2.5 ATR stop (REDUCED from 3.0)
        ...
        # Return wider of: fixed stoploss (-5%) or ATR-based stop
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
                    ...
                    **kwargs) -> Optional[str]:
        """
        FASTER tiered exits (Optimized).
        
        Exit tiers:
        - 6%+ profit: Full exit (REDUCED from 8%)
        - 4%+ profit after 12h: Full exit (REDUCED from 5%/16h)
        """
        ...
        if current_profit >= 0.04:
            ...
            if trade_duration >= 12:  # 3 x 4h candles (REDUCED from 4)
