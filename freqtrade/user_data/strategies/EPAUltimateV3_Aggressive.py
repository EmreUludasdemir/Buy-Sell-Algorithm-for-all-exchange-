"""
EPA Ultimate Strategy V3 - Aggressive ROI Optimization
=======================================================
Variant of EPAUltimateV3 with aggressive profit-taking:

1. Faster ROI table to lock profits quicker
2. Pair stake scaling for risk management
3. Optimized for higher trade frequency

Author: Emre Uludaşdemir
Version: 3.5.0 - Aggressive ROI + Pair Scaling
"""
...
class EPAUltimateV3_Aggressive(IStrategy):
    """
    EPA Ultimate Strategy V3 - Aggressive Profit-Taking Variant
    
    Changes from base V3:
    1. Faster ROI table (8% at entry, scales down quickly)
    2. Pair-specific stake scaling (reduce BTC exposure, cautious on new pairs)
    3. Same entry/exit logic as V3
    """
    ...
    # AGGRESSIVE ROI table - lock profits faster
    minimal_roi = {
        "0": 0.08,       # 8% immediate (was 12%)
        "180": 0.05,     # 5% after 3h (was 8% at 6h)
        "360": 0.03,     # 3% after 6h (was 5% at 12h)
        "720": 0.02,     # 2% after 12h (was 3% at 24h)
        "1440": 0.015,   # 1.5% after 24h (was 2% at 48h)
    }
    ...
    # ==================== PAIR STAKE SCALING ====================
    use_pair_stake_scaling = BooleanParameter(default=True, space='buy', optimize=False)
    
    # Pair-specific multipliers
    PAIR_STAKE_MULTIPLIERS = {
        'BTC/USDT': 0.7,    # Reduce BTC exposure (choppy performer)
        'DOGE/USDT': 0.8,   # New pair - cautious
        'AVAX/USDT': 0.8,   # New pair - cautious
        'LINK/USDT': 0.8,   # New pair - cautious
        'ADA/USDT': 0.8,    # New pair - cautious
        'MATIC/USDT': 0.8,  # New pair - cautious
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime,
        ...
        """Dynamic position sizing with pair-specific scaling."""
        dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
        ...
        # ==================== PAIR STAKE SCALING ====================
        if self.use_pair_stake_scaling.value:
            pair_mult = self.PAIR_STAKE_MULTIPLIERS.get(pair, 1.0)
            risk_amount *= pair_mult
