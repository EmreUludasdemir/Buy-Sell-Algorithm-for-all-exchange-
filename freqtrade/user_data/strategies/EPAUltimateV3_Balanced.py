"""
EPA Ultimate Strategy V3 - OPTIMIZED V2 (Balanced)
===================================================
Based on EPAUltimateV3 with BALANCED parameter optimization.

CHANGES FROM V3:
1. KEPT stoploss: -0.08 (altcoins need room to breathe)
2. Aggressive ROI table: 18%/12%/08%/04% stepped
3. Trailing stop ENABLED for profit lock-in
4. Slightly relaxed protections

LESSON LEARNED: 5% stoploss too tight for altcoins!

Author: Emre Uludaşdemir
Version: 3.4.2-Balanced - Realistic Quick Wins
"""
...
class EPAUltimateV3_Balanced(IStrategy):
    """
    EPA Ultimate Strategy V3 - BALANCED VERSION
    
    Quick Wins Applied (Conservative):
    - ORIGINAL stoploss (8%) - Altcoins need room!
    - Aggressive ROI table - Higher profit targets
    ...
    # ==================== BALANCED ROI TABLE ====================
    # More realistic targets for altcoins
    minimal_roi = {
        "0": 0.18,      # 18% target in first 4 hours (was 12%)
        "180": 0.12,    # 12% after 7.5 hours
        "360": 0.08,    # 8% after 15 hours  
        "720": 0.04,    # 4% after 30 hours
    }

    # ==================== KEEP ORIGINAL STOPLOSS ====================
    # 8% is appropriate for altcoin volatility!
    stoploss = -0.08

    # Enable trailing stop for profit lock-in
    use_custom_stoploss = True
    
    # Trailing configuration - MODERATE
    trailing_stop = True
    trailing_stop_positive = 0.025       # Trail at 2.5% profit
        ...
        return [
            {
                ...
                "stop_duration_candles": 8  # REDUCED: 12 → 8
            },
            {
    ...
    min_kivanc_signals = IntParameter(2, 3, default=2, space='buy', optimize=True)
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        """Dynamic stop loss - keep 8% baseline for altcoins."""
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
                    current_rate: float, current_profit: float,
                    **kwargs) -> Optional[str]:
        """Tiered exits - balanced."""
        if current_profit >= 0.07:
            return 'tiered_tp_7pct'
        
        if current_profit >= 0.04:
            trade_duration = (current_time - trade.open_date_utc).total_seconds() / 3600
            if trade_duration >= 14:
