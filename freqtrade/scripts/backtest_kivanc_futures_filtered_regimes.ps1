$run = "kivanc_futures_filtered_1d_regimes_{0}" -f (Get-Date -Format "yyyyMMdd_HHmmss")
$out = "reports/$run"
...
latest = sorted([p for p in root.iterdir() if p.is_dir() and p.name.startswith("kivanc_futures_filtered_1d_regimes_")], key=lambda p: p.stat().st_mtime)[-1]
scenario_ids = [
    "bear_2022",
    "recovery_2023",
    "bull_2024",
...
summary_path = latest / "kivanc_futures_filtered_1d_regimes_summary.json"
...
(latest / "kivanc_futures_filtered_1d_regimes_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
print(latest)
'@ | python -
