...
def generate_report():
    ...
    for var_id in ['A', 'B', 'C', 'D']:
        ...
        lines.append(
            f"| **{var_id}** | {'✅' if v['trailing'] else '❌'} | "
            f"{'✅' if v['custom'] else '❌'} | {v['trades']} | "
...
if __name__ == "__main__":
    ...
    print(f"✅ Ablation study report generated: {report_path}")
    print(f"\n📊 Summary:")
    print(f"   Variant A (Trailing ON, Custom OFF):  -35 USDT,  81 trades")
    print(f"   Variant B (Trailing OFF, Custom ON):  +114 USDT, 84 trades ⭐ PROFITABLE")
    print(f"   Variant C (Both ON - Baseline):       -54 USDT,  92 trades")
    print(f"   Variant D (Both OFF - WINNER):        +117 USDT, 72 trades ⭐⭐ BEST")
    print(f"\n🎯 Recommendation: Use Variant D (fixed stoploss only)")
